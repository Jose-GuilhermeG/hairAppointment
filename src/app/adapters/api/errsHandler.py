...
async def unauthorize_exception_handler(request : Request , exc : UnauthorizedException)->JSONResponse:
    return JSONResponse(
        status_code=status.HTTP_401_UNAUTHORIZED,
        content={"err" : exc.message}
    )
async def validate_exception_handler(request : Request , exc : ValidateException ) -> JSONResponse:
    return JSONResponse(
        status_code=status.HTTP_400_BAD_REQUEST,
        content={
            "err" : exc.message
